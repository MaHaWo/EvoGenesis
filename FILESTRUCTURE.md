...
* Python source files:
  * The Python package source files are located in the `EvoGenesis` directory.
  * The file `EvoGenesis.__main__.py` defines a command line interface that
    is accessible both as the command `EvoGenesis` and via
    `python -m EvoGenesis`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_EvoGenesis.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `EvoGenesis`.
